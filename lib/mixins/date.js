...
module.exports = Controller => class extends Controller {
    ...
    validateDateField(req, fieldName, errors) {
        ...
        let fieldErrors = _.pick(errors, error => error.errorGroup === fieldName);
        let numericErrors = _.pick(fieldErrors, error => error.type === 'numeric');
        if (!_.isEmpty(numericErrors)) {
            let errorType = 'date-numeric';
            if (numericErrors[fieldName + '-year']) errorType += '-year';
            if (numericErrors[fieldName + '-month']) errorType += '-month';
            if (numericErrors[fieldName + '-day']) errorType += '-day';
